...
class Program
{
    ...
    //    {
    //        string logFile = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, $"UsageReportLog_{DateTime.Now:yyyyMMdd}.txt");

    //        void Log(string message)
    //        {
    //            string logLine = $"{DateTime.Now:MM/dd/yyyy HH:mm:ss} {message}";
    ...
    //                // Paths
    //                string dataBilledPath = null, dataUnbilledPath = null;
    //                string smsBilledPath = null, smsUnbilledPath = null;
    ...
    //                  ('DataUsageBilledPath','DataUsageUnbilledPath','SMSUsageBilledPath','SMSUsageUnbilledPath')";

    //                using (SqlCommand cmd = new SqlCommand(pathQuery, conn))
    //                using (SqlDataReader rdr = cmd.ExecuteReader())
    //                {
    //                    while (rdr.Read())
    //                    {
    //                        string name = rdr["setting_name"].ToString();
    //                        string value = rdr["setting_value"].ToString();

    //                        switch (name)
    //                        {
    //                            case "DataUsageBilledPath": dataBilledPath = value; break;
    //                            case "DataUsageUnbilledPath": dataUnbilledPath = value; break;
    //                            case "SMSUsageBilledPath": smsBilledPath = value; break;
    //                            case "SMSUsageUnbilledPath": smsUnbilledPath = value; break;
    //                        }
    //                    }
    //                }

    //                if (string.IsNullOrWhiteSpace(dataBilledPath) || string.IsNullOrWhiteSpace(dataUnbilledPath)
    //                    || string.IsNullOrWhiteSpace(smsBilledPath) || string.IsNullOrWhiteSpace(smsUnbilledPath))
    //                {
    //                    throw new Exception("One or more Data/SMS billed/unbilled paths are not configured in tbl_settings.");
    //                }
    ...
    //                        // Update/delete old files and DB for both Data and SMS
    //                        DeleteOldCsvFilesAndUpdateDb(dataBilledPath, prevCycleEnd.AddDays(1), "data_usage_filename", "data_usage_filename_update_date", "old_data_usage_filename");
    ...
    //                        DeleteOldCsvFilesAndUpdateDb(smsBilledPath, prevCycleEnd.AddDays(1), "text_usage_filename", "text_usage_filename_update_date", "old_text_usage_filename");
    ...
    //                    Log($"[UPDATED-DATA] Unbilled CSV updated for reseller {reseller.Name} - {newDataFileName}");
